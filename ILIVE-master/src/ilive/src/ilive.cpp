...
{
    printf_program("ILIVE: A Robust, Real-time, RGB-colored, LiDAR-Inertial-Visual tightly-coupled state Estimation and mapping package");
    Common_tools::printf_software_version();
    Eigen::initParallel();
    ros::init(argc, argv, "ILIVE_main");
    ILIVE * fast_lio_instance = new ILIVE();
    ros::Rate rate(5000);
    bool status = ros::ok();
    ros::spin();
}
