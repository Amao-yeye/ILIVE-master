...
ImuProcess::ImuProcess() : b_first_frame_( true ), imu_need_init_( true ), last_imu_( nullptr ), start_timestamp_( -1 )
{
    ...
    //ext_t_lid_in_imu = Eigen::Vector3d(0.0582, -0.2168, -0.0324);
...
// fout<<head->header.stamp.toSec()<<" "<<angvel_avr.transpose()<<" "<<acc_avr.transpose()<<std::endl;
    ...
    Eigen::Vector3d pos_liD_e = state_inout.rot_end* R_lidar_in_IMU * (p_body + ext_t_lid_in_imu) + state_inout.pos_end+state_inout.rot_end*ext_t_lid_in_imu
    //Eigen::Vector3d pos_liD_e = state_inout.pos_end + state_inout.rot_end * ext_t_lid_in_imu;
    ...
    for ( auto it_kp = IMU_pose.end() - 1; it_kp != IMU_pose.begin(); it_kp-- )
    {
        ...
        for ( ; it_pcl->curvature / double( 1000 ) > head->offset_time; it_pcl-- )
        {
            ...
            //Eigen::Vector3d T_ei( pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt + R_i * ext_t_lid_in_imu - pos_liD_e );
            ...
            Eigen::Vector3d P_compensate = ext_R_lid_in_imu * (state_inout.rot_end * (R_i * (ext_R_lid_in_imu * P_i + ext_t_lid_in_imu) + T_ei) - ext_R_lid_in_imu); // not accurate!
